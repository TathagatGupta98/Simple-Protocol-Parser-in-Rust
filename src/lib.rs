...
fn parse(input: &str)-> Result<Command, Error>{
    ...
    let Some(req) = input.strip_suffix('\n') else{
        return Err(Error::IncompleteMessage)
    };

    if true == req.contains('\n'){
    ...
    if let Some(url) = req.strip_prefix("PUBLISH ") {
        Ok(Command::Publish(String::from(url)))
    } else if req == "PUBLISH"{
        Err(Error::MissingPayload)
    } else if req == "RETRIEVE" {
        Ok(Command::Retrieve)
    } else if let Some(url) = req.strip_prefix("RETRIEVE ") {
